...
오큐브 홈페이지의 **정본 저장소**입니다. `docs/non-developer-handoff-notes.txt` 의 지침대로
리팩터링한 판이며, 2026-08-13 사용자 확정으로 정본이 됐습니다.
...
> **2026-08-13 이 저장소가 정본이 됐습니다.** 먼저 만든 판(`ocube-homepage-next`)은 GitHub 에서
> 보관 처리(읽기 전용)했습니다. 콘텐츠 변경도 앞으로 여기서 합니다.
> 보관본의 공개 주소는 그대로 살아 있으니(내용이 같습니다), 이미 공유한 링크는 깨지지 않습니다.
...
npm run build
npm run verify              # → 검사 576건 중 통과 576건. 여기서 시작한다
```
...
4. 되돌린다 → `npm run build && npm run verify` → 다시 576/576.
...
npm run build
npm run verify        # DOM 대조 48쪽 x 12항목 = 576건
...
**기준선을 먼저 고치지 않는다.** 순수 리팩터링이면 `verify` 576 건과 스크린샷이
...
| body-extra                              | `<body>` 바로 아래의 나머지 요소(토스트·맨 위로 버튼·스크롤 진행 바) — 위 검사들이 아무도 보지 않던 자리다                                        |
...
**현재 상태: 48페이지 × 12항목 = 576/576 통과.**
콘텐츠를 손대면 이 게이트가 즉시 깨지므로, 의도하지 않은 회귀를 막는 안전망입니다.
데이터화 같은 순수 리팩터링은 **기준선 갱신 없이 576 이 그대로 통과**해야 한다 —
그것이 DOM 무변경의 증명이다.
...
- [~] 좁은 화면(≤1150px) 헤더 메뉴 ↔ KR/EN 겹침 — **지금대로 두기로 결정(2026-08-13)**.
  두 판 공통의 기존 상태다. 다시 다루려면 `docs/pitfalls.md` 2절의 선택지부터 정할 것
