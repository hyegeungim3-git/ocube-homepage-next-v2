...
### 마지막 적대 검증 (2026-08-13)

수정을 마친 뒤, 원본과 최신 v2 를 다시 한번 **적대적으로** 대조했다. 판정은 **회귀 없음**이다.

| 축                                  | 규모      | 결과                                               |
| ----------------------------------- | --------- | -------------------------------------------------- |
| 정적 HTML 보이는 글자(공백까지)     | 46쪽      | 차이 **0**                                         |
| head(title·meta·canonical·hreflang) | 46쪽      | 스타일시트 교체 외 **0**                           |
| 픽셀 전체 페이지, 허용오차 **0**    | 92쌍      | 실차이 **0**(뜬 2건은 아티팩트로 확정)             |
| 하이드레이션 후 DOM                 | 92 스냅샷 | 차이 전량이 의도 6종에 대응, **설명 안 되는 것 0** |
| 런타임 건강                         | 92셀      | 양 빌드 클린                                       |
| 대비(조상 알파 합성)                | 394건     | 두 빌드 **완전 동일**                              |
| 자산 바이트                         | 271개     | 의도한 2개 제거 외 개행문자뿐                      |

**아티팩트로 판정한 것** — index 히어로 차이(1440 1,258px)는 타자기 진행 상태였고 글자를
확정 상태로 굳히자 완전 동일. about 의 1픽셀(1/255)은 요소 단위로 다시 찍으니 동일해
전체 페이지 합성 아티팩트로 확정. **같은 빌드를 두 번 찍어 잡음 바닥을 먼저 재 두면**
이런 판정이 쉬워진다.

**남은 부수 변화 3건**(전부 눈에 안 보임): 개행문자(정리함) · React 텍스트노드 병합으로
글자 위치가 1/64px 이동(`business-si` h2 의 `<em>` 시작 x 37.03125 → 37.01563) ·
아무도 참조하지 않는 `main#main-content` 소실.

**검증 도구의 한계도 함께 기록한다** — 첫 DOM 직렬화기가 공백만 있는 텍스트노드를 버려
위 '텍스트노드 병합' 을 가렸고, 공백까지 정확히 비교하는 검사를 따로 돌리고서야 드러났다.
