...
**2026-08-13 부터 이 저장소가 정본이다.** 같은 원본에서 갈라진 판이 셋 있고, 나머지는 보관용이다.

|              | 저장소                                   | 공개 주소                 | 상태                |
| ------------ | ---------------------------------------- | ------------------------- | ------------------- |
| **정본**     | `ocube-homepage-next-v2`                 | …/ocube-homepage-next-v2/ | 여기서 고친다       |
| 먼저 만든 판 | `ocube-homepage-next`                    | …/ocube-homepage-next/    | **보관(읽기 전용)** |
| 그 이전      | `ocube-homepage-final` · Cloudflare `a/` | …/ocube-homepage-final/   | 갱신 안 함          |

정본이 바뀌기 전에 **두 판의 화면·기능이 같다는 것을 확인했다** — 46쪽을 브라우저로 여러 각도에서
대조했고 방법과 수치는 `refactoring-roadmap.md` 의 "원본과의 대조 검수"·"마지막 적대 검증" 절에 있다.

보관본의 공개 주소는 살려 둔다(내용이 같아 이미 공유한 링크가 깨지지 않는다). 대신 저장소를
읽기 전용으로 두어 **두 곳이 갈라지는 것을 막는다.**
