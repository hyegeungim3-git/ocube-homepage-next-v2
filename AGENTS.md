...
- [ ] 로드맵 3단계 다국어 코드 통합 — **아직 시작 안 함** (아래 인수인계 참조)
...
**3단계(다국어 통합)가 로드맵 한 줄보다 훨씬 크다.** 시작 전에 규모를 알고 들어갈 것.

| 실측                               | 값                                                                 |
| ---------------------------------- | ------------------------------------------------------------------ |
| 화면에 그대로 보이는 한국어 덩어리 | 23쪽 **1,369개 · 27,012자**                                        |
| 그중 사전이 이미 가진 것           | 853개 (나머지 516개는 `<wbr />` 로 갈라져 있어 이어 붙여야 맞는다) |
| 한국어 줄바꿈 힌트 `<wbr />`       | 272곳                                                              |
| 합쳐야 할 데이터 짝                | `src/data/*.ts` ↔ `*.en.ts` 16쌍                                   |

걸림돌은 분량이 아니라 **`<wbr />`** 다. 한국어는 `"산업 통신 규격(OPC-UA·" <wbr/> "Modbus)"`
처럼 문장이 갈라져 있고, 영어는 그것을 이어 붙인 **한 문장**으로 번역돼 있다. 그래서
`t(lang, "…")` 같은 실행 시점 사전 조회로는 영어 문장을 못 맞춘다 — 갈라진 조각을 각각
찾게 되기 때문이다. 이미 있는 `RichToken` 배열(`"wbr"` 토큰 포함)로 문구를 옮겨야 두 언어가
같은 자료에서 나온다. 즉 3단계는 사실상 **본문 데이터화를 끝내는 일**이다.
`src/data` 16쌍을 `Record<Lang, T>` 로 합치는 것부터 시작하면 위험이 가장 낮다.
