...
다음 한 개를 고른다면 **`08-hero-slider`** 다. `HomeSlides` 가 이미 React 라 상태로 옮기기
가장 자연스럽고, 슬라이더 TC 8건이 이미 동작을 붙잡고 있다. `07-nav-current` 은 반드시
`06-mobile-menu` **뒤에** — 아코디언이 만들어진 뒤에 현재 위치를 표시해야 한다.
