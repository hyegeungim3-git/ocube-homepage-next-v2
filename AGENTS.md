...
- 아직 옮기지 못한 것만 `public/assets/site2.js`(바닐라)에 남아 있다(8개 파일).
...
- [~] 로드맵 6단계 `site2.js` React 이전 — **18/25 모듈** (`site2.js` 44KB → 20KB)

한 단계씩 진행하고, 통과한 뒤에 다음 단계로 넘어간다. 여러 단계를 한 변경에 섞지 않는다.

### 다음 사람에게 — 남은 것은 6단계뿐

**6단계는 절차가 이미 검증돼 있다.** 18개를 그 순서로 옮겼고, 순서와 남은 목록은
`docs/refactoring-roadmap.md` 6절에 있다. 남은 6개 기능도 같은 순서를 반복하면 된다.
`tests/e2e` 에 메뉴·필터·슬라이더·복사·라이트박스·영상·표 감싸기·리빌·도트의 동작이
이미 고정돼 있어, 옮긴 뒤 **기대값을 고치지 않고** 통과하는지로 판정할 수 있다.

다음 한 개를 고른다면 **`08-hero-slider`** 다. 슬라이더 TC 8건이 이미 동작을 붙잡고 있다.
⚠️ 다만 마크업이 두 곳에 나뉘어 있다 — 슬라이드는 `HomeSlides`, 좌우 버튼·진행 바는
`index-page` 안이다. **홈 히어로 구역 전체를 클라이언트 컴포넌트 하나로 감싸는 것이 먼저**고,
그때 DOM 이 달라지지 않게 조심해야 한다.

`07-nav-current` 은 반드시 `06-mobile-menu` **뒤에** — 아코디언이 만들어진 뒤에 현재 위치를
표시해야 한다. `25-contact-mail` 은 실제 전송 방식이 결정 대기 항목이라 확인이 먼저다.
