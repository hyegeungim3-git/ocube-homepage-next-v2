...
- [~] 로드맵 6단계 `site2.js` React 이전 — **1/25 모듈** (맨 위로 버튼)

한 단계씩 진행하고, 통과한 뒤에 다음 단계로 넘어간다. 여러 단계를 한 변경에 섞지 않는다.

### 다음 사람에게 — 남은 것은 6단계뿐

**6단계는 절차가 이미 검증돼 있다.** 첫 기능에서 확인한 6단계 순서를
`docs/refactoring-roadmap.md` 6절에 적어 두었다. 남은 24개 모듈에 같은 순서를 반복하면 된다.
`tests/e2e` 에 메뉴·필터·슬라이더·복사·라이트박스·영상·표 감싸기의 동작이 이미 고정돼 있어,
옮긴 뒤 **기대값을 고치지 않고** 통과하는지로 판정할 수 있다.
