...
export default defineConfig({
  ...
  expect: {
    ...
    toHaveScreenshot: {
      ...
      //
      // ⚠️ 비율은 **면적에 비례**한다. 0.002 로 두었더니 홈 데스크톱(1440x6182 = 890만 px)에서
      // 17,804 px 까지 허용돼 **푸터 주소 세 줄이 통째로 바뀌어도 초록**으로 지나갔다.
      // 같은 변경이 면적이 작은 모바일에서는 걸렸다 — 즉 큰 화면일수록 눈이 멀었다.
      // 픽셀별 차이는 아래 threshold 가 이미 걸러 주므로 비율은 낮게 잡는다.
      // (기준 이미지는 같은 기계에서 만들므로 안티에일리어싱 차이는 사실상 0 이다)
